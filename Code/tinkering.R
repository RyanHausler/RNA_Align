...
ls <- list.files("C:/Users/Ryan/Documents/Wright/Seeds/") # use all files

k <- which(ls=="RF00005.txt") # use a single file

# ls <- readLines("~/Desktop/files.txt") # do a subset of files

# looping start
threshs <- seq(0, 1, 0.01)
AUCs <- Qs <- heights <- corrs <- threshold <- numeric(length(ls))
sensitivities <- PPVs <- numeric(length(threshs))
Ws <- integer(length(ls))

results <- vector("list", length(ls))
...
rna <- readRNAStringSet("C:/Users/Ryan/Documents/Wright/Seeds/RF00005.txt")
...
d <- DistanceMatrix(rna, verbose=FALSE, includeTerminalGaps=TRUE)
...
#####################################################
## Compute the number of mutations required
#####################################################

.Fitch <- function(tree) {
	if (is.leaf(tree)) {
		pos <- pairs[unlist(tree)]
		pos <- match(pos, paired)
		if (is.na(pos))
			pos <- 7L
		z <- rep(0L, 8)
		z[pos] <- 1L
		...
		w <- which(l[1:6] > 0 & r[1:6] > 0)
		if (length(w) > 0) { # pairing intersection
			z <- rep(0L, 8)
			z[w] <- 1L
			z[8] <- l[8] + r[8]
			return(z)
...
str <- readLines(paste0("~/Desktop/Alignment/RNABenchmarks/Rfam_Seeds/wuss/", substring(ls[k], 1, nchar(ls[k]) - 3), ".txt"))
s <- strsplit(str, "")[[1]]
c1 <- c2 <- integer()
